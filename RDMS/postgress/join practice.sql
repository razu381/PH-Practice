...
SELECT avg(salary),departments.department_name from employees
join departments on departments.department_id=employees.department_id
GROUP BY departments.department_name

SELECT count(*),departments.department_id,departments.department_name from employees
JOIN departments on departments.department_id = employees.department_id
GROUP BY departments.department_name,departments.department_id

SELECT avg(salary) as avg_salary,departments.department_name from employees
join departments USING(department_id)
GROUP BY departments.department_name
ORDER BY  avg_salary DESC
LIMIT 1 

SELECT  extract("Year" from employees.hire_date) as hiring_year,count(*) from employees
